...
# --- AYARLAR ---
# Render'da Environment Variables kısmına DISCORD_TOKEN eklemeyi unutma knk!
...
@bot.event
async def on_ready():
    print(f'Render Nöbetçi Botu ({bot.user}) Aktif!')

@bot.command()
async def panel(ctx):
    await ctx.send("🎮 **FETTAH REMOTE (NÖBETÇİ)**\nPC kapalıysa 'PC AÇ' butonuna bas knk.", view=ControlPanel())
